...
function [cMass] = bigCrunchPhase_eq1(pop)
    ...
    [normalizedSwarm, reference_directions] = doNormalize(pareto(:, :));
    assosiations = assosiate(normalizedSwarm, reference_directions, pop(:, :));

    elitesLeftToChoose = bbbcs.n_cmass;
    cMass = zeros(elitesLeftToChoose, width(pop));
    
    while true
        if elitesLeftToChoose == 0
            break;
        end
        ...
        for i = 1:width(assosiations)
            ...
            if(assosiations(i).count <= 1)
                continue;
            end
            targetRank = assosiations(i).pop(1).position(bbbcs.rankIndex);
            shortestIndex = 1;
            for j = 2:assosiations(i).count - 1
                if(assosiations(i).pop(j).position(bbbcs.rankIndex) == targetRank)
                    if(assosiations(i).pop(shortestIndex).dist >= assosiations(i).pop(j).dist)
                        if(assosiations(i).pop(shortestIndex).dist == assosiations(i).pop(j).dist && rand() >= 0.5)
                            continue;
                        end
                        shortestIndex = j;
                    end
            ...
            cMass(elitesLeftToChoose, :) = assosiations(i).pop(shortestIndex).position;
            elitesLeftToChoose = elitesLeftToChoose - 1;
