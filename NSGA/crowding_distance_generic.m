...
for i=1:n
    ...
    for m=2:l-1
        sortedFront(m,DistanceIndex)= (sortedFront(m,DistanceIndex)+ (sortedFront(m+1,1:k)-sortedFront(m-1,1:k))/(max(fit_array(:,1:k))-min(fit_array(:,1:k))));
    end
    ...
    %obj1 (index1)
%     front=[front (1:size(front, 1))'];
%     sortedFront=[sortrows(front,obj1)];
% 
%     sortedFront(1,DistanceIndex)=inf;
%     sortedFront(end,DistanceIndex)=inf;
%     for m=2:l-1
%         sortedFront(m,DistanceIndex)= (sortedFront(m,DistanceIndex)+ (sortedFront(m+1,obj1)-sortedFront(m-1,obj1))/(max(fit_array(:,obj1))-min(fit_array(:,obj1))));
%     end
% 
%     sortedFront=[sortrows(sortedFront,size(sortedFront,2))];
%     front=sortedFront(:,1:end-1);
    ...
    %obj2 
%     front=[front (1:size(front, 1))'];
%     sortedFront=[sortrows(front,obj2)];
% 
%     sortedFront(1,DistanceIndex)=inf;
%     sortedFront(end,DistanceIndex)=inf;
%     for m=2:l-1
%         sortedFront(m,DistanceIndex)= (sortedFront(m,DistanceIndex)+ (sortedFront(m+1,obj2)-sortedFront(m-1,obj2))/(max(fit_array(:,obj2))-min(fit_array(:,obj2))));
%     end
% 
%     sortedFront=[sortrows(sortedFront,size(sortedFront,2))];
%     front=sortedFront(:,1:end-1);
